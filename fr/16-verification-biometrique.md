...
- pour qu'une même personne ne puisse pas détenir deux passeports valides, seules des empreintes cryptographiques irréversibles sont conservées;
- revenir après une sortie est possible à tout moment;
- un pseudonyme au lieu du nom réel: c'est votre choix;
- la biométrie sert la confiance, non le contrôle;
- système de vérification propre, séparation des espaces de stockage, minimisation des données.
...
Elle sert un seul but: attester que derrière chaque voix se tient une seule personne vivante et unique. C'est le socle de la confiance entre inconnus, et rien de plus. Le système est conçu de manière à ne pas pouvoir être employé à des fins de surveillance.
...
**1. Vérifier l'unicité, non contrôler.** La vérification protège le peuple contre les inscriptions multiples, mais ne crée pas de base de surveillance.
...
**4. La protection contre les abus.** La séparation des espaces de stockage, le chiffrement et l'impossibilité pratique de reconstituer une image à partir des données conservées sont conçus pour que le système ne puisse pas servir à une surveillance de masse.

---

# SECTION 02. Champ d'application et consentement

La présente Politique détermine le traitement des données biométriques lors de la signature de la Déclaration, de l'obtention du statut d'earthling et de la participation à l'infrastructure du peuple.
...
- **le contrôle du document et du visage**.

**Les nom et prénom réels ne sont pas conservés.** Les données du document ne sont utilisées qu'au moment de la vérification - pour comparer le visage au document et confirmer l'unicité - et sont supprimées une fois celle-ci achevée. Votre pseudonyme demeure votre nom public.
...
**Document → visage → contrôle de présence vivante → comparaison document/visage → résultat → conservation protégée du résultat**
...
- **la présence vivante** - protection contre les photographies, les enregistrements vidéo et les masques. Le contrôle suit le modèle de détection des attaques par présentation décrit dans la norme ISO/IEC 30107; le niveau de résistance annoncé et les résultats d'une vérification indépendante seront publiés lors de la mise en exploitation industrielle du système.
...
6. **Conservation du résultat** - sous forme chiffrée.

> **Ce qui subsiste après la vérification.** Les photographies, les scans de documents et les gabarits biométriques **ne sont pas conservés**. Subsistent: l'état de la vérification (réussie ou non) et des empreintes cryptographiques irréversibles, calculées à partir du numéro du document et de la combinaison du nom et du pays.
>
> Les empreintes n'interdisent pas de revenir. Elles empêchent seulement une même personne de détenir deux passeports valides en même temps: lors d'une nouvelle adhésion, le système trouve la correspondance, s'assure que l'ancien passeport a été détruit et en délivre un nouveau.

> **Précisément, sur le statut des empreintes.** Une empreinte est irréversible: on ne peut pas en tirer un nom ou un numéro de document. Mais elle permet de **distinguer une personne déterminée** des autres, sans quoi elle ne remplirait pas sa tâche. Selon le RGPD, il s'agit donc de données **pseudonymisées et non anonymes**, et la protection des données personnelles leur est applicable dans son intégralité. Nous ne les qualifions pas d'anonymisées, car ce serait inexact.
...
**Chiffrement au repos.** Les empreintes sont conservées sous forme chiffrée (AES-256). Les clés de déchiffrement sont conservées séparément des données.

**Séparation des espaces de stockage.** Les empreintes et les résultats de vérification sont conservés séparément des données du compte.
...
**Contrôle des accès.** Authentification à plusieurs niveaux et journalisation: tous les accès aux données de vérification sont consignés et peuvent être vérifiés.

> **Philosophie de la sécurité:** la meilleure protection, c'est quand il n'y a rien à voler. Nous ne conservons pas ce qui pourrait être employé contre vous.
...
- le lien entre les données de vérification et votre identité est rompu;
- les empreintes pseudonymisées sont conservées exclusivement pour qu'une même personne ne puisse pas détenir deux passeports valides;
...
- reconstituer une image à partir des empreintes ou identifier une personne est pratiquement impossible.
...
Le code du contrat intelligent du passeport est ouvert sous licence MIT et vérifiable dans l'explorateur du réseau.
...
- la **journalisation des accès** aux données de vérification est tenue et peut être auditée.
...
**Ce qui est conservé:** le pseudonyme; l'adresse électronique; la confirmation d'avoir 18 ans; le pays de résidence (à des fins statistiques); l'état de la vérification d'identité; le lien avec le passeport; la date d'obtention du statut.

## La minimisation

Le registre suit le principe de minimisation des données conformément au RGPD. N'est conservé que le nécessaire: l'unicité vérifiée, le lien avec le passeport pour la participation aux décisions, un identifiant interne pour la répartition des rémunérations.
...
Non. Le gabarit biométrique n'est pas conservé du tout: la comparaison s'effectue au moment de la vérification, après quoi les données d'origine sont supprimées. Subsistent des empreintes irréversibles, dont on ne peut tirer ni image ni données de document.

**Que se passe-t-il si je perds mon téléphone?**
Les données de vérification sont en sécurité. Pour retrouver l'accès, il suffit d'installer l'application sur un nouvel appareil et de repasser la vérification.

**Ma biométrie peut-elle être dérobée?**
On ne peut dérober que des empreintes chiffrées et irréversibles, inutiles sans les clés de déchiffrement. Reconstituer une image de visage à partir d'elles est pratiquement impossible.

**Faut-il obligatoirement indiquer son vrai nom?**
Non. Les nom et prénom réels ne sont pas conservés. Les données du document ne sont contrôlées qu'au moment de la vérification et sont supprimées ensuite. Dans les échanges quotidiens, vous êtes connu par votre pseudonyme.

**Qu'advient-il des données à la sortie?**
Le lien entre les données de vérification et votre identité est rompu. Les empreintes pseudonymisées sont conservées exclusivement pour qu'une même personne ne puisse pas détenir deux passeports valides. Cela n'empêche pas de revenir.
...
Vous recevrez une notification des motifs. Vous pouvez recommencer après les avoir levés, par exemple avec des images de meilleure qualité ou un autre document. En cas de désaccord, vous avez le droit d'exiger un réexamen par un être humain, et après deux tentatives automatiques infructueuses ce réexamen a lieu automatiquement.
