...
**One possible model of the future. Not a blueprint to be rolled out - an example of how the workings of our common home can be taken apart and tested in the first place.**
...
At the base lies a radical but productive metaphor: the present world order - with its whole social, political, economic, and legal fabric - is a working but old operating system. Call it "Windows XP" (a Microsoft trademark; the name is used as a metaphor, not as a statement about the product). It is not meaningless: it boots, and billions of processes live on it. But its bugs are already known - the ones that have shown themselves over decades and cost human lives.

The question of this document: given a full staff of developers and a blank sheet, what would the next system look like - "Terra 1.0"? What is meant is not another release from the same vendor: the new system is assembled by other people, and it has no owner. There is no ideal system - what is meant is the most correct and complete version achievable in the present situation.

The OS metaphor is taken seriously. An operating system has a real anatomy: the kernel and privilege rings, the permission model, process isolation, the scheduler, the update mechanism, error handling, authentication. Each axis maps onto the workings of a society with surprising precision - and where the mapping breaks, it breaks instructively. At the end (Part IX) the chief flaw of the metaphor itself is also examined: an OS has an owner, and humanity must not have one. The language of operating systems was chosen for exactly that precision - it is the closest and clearest way to explain a design of this kind. At the same time "Terra 1.0" is an analytical lens, not a slogan: in the model itself the state does not disappear but becomes a thin layer (Part III), so what is meant is the rebuilding of the whole stack as a subject of examination, complementing states rather than abolishing them.
...
# Part I. Diagnosis: the bugs of "Windows XP"
...
# Part III. The architecture of "Terra 1.0"

III.1

## A microkernel instead of a monolith

The first decision of any OS: what runs in ring 0 (privileged) and what runs in user space, where a process can crash without bringing the system down. A monolith is bad architecture. Here the architecture is a **microkernel**. In the kernel is only what one cannot exit: what is physically inseparable and rival - and the protection of those who cannot contribute (Part II):

- protection of physical safety and physical space, and the protected floor - the guaranteed minimum below which the system lets no one fall (Module 2);
- the planetary life-support systems - climate, ocean, atmosphere, orbit, spectrum, water;
- governance of the supertechnologies where the cost of an error is the whole species (artificial intelligence, bioengineering);
...
- **Keep the common in working order.** Participation as maintenance. An OS no one maintains degrades. Citizenship is both a login and a duty shift on the system: the minimal share of attention and labour without which the common rusts.
...
- **Plurality of issuers instead of a monopoly.** Many independent ones, k-of-n sufficing. None is root, none is a single point of exclusion. The price of this move is named plainly: to stop one person obtaining a proof from several issuers at once, the issuers need a shared linkable space of identifiers - and that is the same root under another name; without it, uniqueness holds only within each issuer. Here the scheme sacrifices strict global uniqueness for the sake of privacy and decentralization.
...
A model not run against a scenario to breaking point is not a model but a stage set. Running "Terra 1.0" through three hard scenarios shows honestly where it falls.
...
>
> A track is a specific piece of work for the common good: you can carry it out yourself, or you can support it with money or with your own hands. The work is done in the open, in small steps, and every step can be checked. The rules for support are strict: the vote is not for sale, support gives no power over people, and no one promises anything in advance. Helping to get a specific piece of work done is allowed. Using money to buy the direction in which the people is going is not.
