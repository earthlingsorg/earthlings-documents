...
SECTIONS = [
    ...
    # Документ 02 - пункт верхнего уровня, вторым, без подменю (решение
    # Артура 2026-08-23). Подписи разной длины намеренно: при 1248px, ниже
    # которого шапка сворачивается в бургер, шестой пункт с полным названием
    # не собирается - французскому не хватает 52 пикселей, испанскому 13.
    # Поэтому у них одно слово, а у ru, en и de полное. Считает
    # _v2/tools/menu_fits.py; глазами это не проверяется, потому что
    # по-русски запас плюс 81 и всё выглядит благополучно.
