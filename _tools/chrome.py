...
EXTRA = {
    ...
    # Подпись кнопки под анонсом Обращения. Заменяет 'read_addresso', когда
    # анонс Обращения написан (_announce/<язык>-announce.md): он не
    # пересказывает текст, а называет причину, и кнопка обязана обещать то же
    # самое. Анонс стоит во всех девяти языках или ни в одном - это проверяет
    # build_home_v2.py, поэтому подпись нужна на всех девяти.
...
#
# Запись - (адрес, подпись). Обе половины принимают две формы:
#
#   'https://...'                     одно значение на все девять языков
#   {'ru': '/a/', '*': '/b/'}         по языкам; '*' - для всех остальных
#
# Словарь понадобился книге: русский текст лежит по /book/ru/, английский по
# /book/en/, и остальным восьми языкам показывается английский.
#
# Книга добавлена 2026-09-06, в день подмены. Условие, записанное здесь
# заранее, выполнено: `book` отдаётся из прежнего дерева через `location
# @shared` в nginx, и все 21 страница отвечают 200 - проверено на живом сайте
# перед тем, как ссылка появилась в подвале.
#
# Подпись на русском русская, на остальных восьми английская, и это не
# недоделка. Перевода названия нет нигде, кроме ru и en, а ссылка на этих
# восьми языках ведёт на АНГЛИЙСКИЙ текст: подпись обязана говорить на языке
# того, что за ней открывается, иначе грузинское название пообещало бы
# грузинскую книгу.
#
# Вторая запись - страница /essays/, добавлена 2026-09-07. Строка ОДНА, а не
# одиннадцать: перечислять на ней статьи поштучно значило бы перекосить
# колонку на всех девяти языках. Адрес один для всех, подпись английская на
# всех девяти - по тому же правилу, по которому книга подписана английским
# названием на восьми: подпись говорит на языке того, что за ссылкой, а
# страница статей английская и переводов у неё нет.
FOOTER_ESSAYS = [
    ({'ru': '/book/ru/', '*': '/book/en/'},
     {'ru': u'Свет в тёмной комнате', '*': u'Light in the Dark Room'}),
    ('/essays/', {'*': u'Articles and essays'}),
]
    ...
    if FOOTER_ESSAYS:
        ...
        for href, label in FOOTER_ESSAYS:
            ...
            if isinstance(href, dict):
                ...
                url = href
            # Подпись - по тому же правилу, что и адрес: язык, потом '*',
            # потом английский. Без '*' запись, у которой подпись общая для
            # восьми языков, роняла бы сборку на KeyError.
            text = label.get(lang) or label.get('*') or label['en']
