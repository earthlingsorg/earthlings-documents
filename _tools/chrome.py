...
def footer_html(lang, doc_href, has_doc=None):
    ...
    # который уже написан, потом два способа спросить живого человека.
    a(u'<div class="ftr-col"><h2 class="ftr-h">%s</h2><ul class="ftr-list">'
      % esc(t(lang, 'footer.contact')))
    if has_doc('27'):
        a(u'<li><a href="%s">%s</a></li>'
          % (esc(doc_href('27')), esc(t(lang, 'nav.faq_general'))))
    ...
    # Внизу - строка прав слева, «О нас» и юридическая информация справа
    # (решение Артура 2026-08-25). Ссылки собраны в одну группу, чтобы их
    # можно было прижать к правому краю одним правилом, а не отступом.
    #
    ...
    # справа. Теперь строка действительно излишня.
