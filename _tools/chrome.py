...
# Переводы обвязки. Лежат в репозитории сборки, а не в дереве сайта:
# после подмены _v2 становится веб-корнем, и всё, что в нём лежит,
# отдаётся наружу. Сборочные данные наружу отдавать незачем.
TRANS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'i18n')
...
EXTRA = {
    ...
    # Подпись кнопки под анонсом Обращения. Заменяет 'read_addresso' на тех
    ...
    'read_addresso': {'ru': u'Читать Обращение', 'en': u'Read the Address',
    ...
    # Колонка эссе в подвале. Состав - в FOOTER_ESSAYS; пока список пуст,
    # колонка не рисуется вовсе.
...
# Эссе в подвале. Место заведено 2026-08-23; 2026-09-02 решением Артура сюда
# же встаёт книга, а рядом с ней будут статьи и эссе.
#
# Запись - (адрес, подпись по языкам). Адрес принимает две формы:
#
#   'https://...'                     один адрес на все девять языков
#   {'ru': '/a/', '*': '/b/'}         по языкам; '*' - для всех остальных
#
# Словарь понадобился книге: русский текст лежит по /book/ru/, английский по
# /book/en/, и остальным восьми языкам показывается английский. Подписи при
# этом свои у каждого языка, поэтому вторая половина записи не меняется.
#
# ВНИМАНИЕ. Ссылка на /book/ заработает только после того, как `book` войдёт в
# общий блок nginx рядом с images|fonts|downloads|awakened_code: книга остаётся
# в старом дереве и в _v2 не переносится (решение Артура 2026-09-02, оформление
# у неё прежнее). Пока этого нет, добавлять сюда книгу И ПЕРЕСОБИРАТЬ страницы
# нельзя - в подвале каждой страницы появится битая ссылка.
FOOTER_ESSAYS = []
...
def footer_html(lang, doc_href, has_doc=None, flush=False):
    ...
    if FOOTER_ESSAYS:
        ...
        for href, label in FOOTER_ESSAYS:
            # Адрес по языку, если он задан словарём. '*' - общий запасной;
            # без него запись с неполным словарём молча дала бы пустой href.
            if isinstance(href, dict):
                url = href.get(lang) or href.get('*') or href['en']
            else:
                url = href
            a(u'<li><a href="%s" rel="noopener">%s</a></li>'
              % (esc(url), esc(label.get(lang) or label['en'])))
        a(u'</ul>')
        a(u'</div>')
