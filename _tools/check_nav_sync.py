...
Проверка перестанет быть нужной, когда новый сайт заменит старый и constants.js
уйдёт. До тех пор её место - в предкоммитном прогоне.

Запуск:  python earthlings-documents/_tools/check_nav_sync.py
Выход:   0 - деревья совпадают, 1 - разошлись.
"""
import io
import os
import re
...
def main():
    ...
    lines.append(u'')
    lines.append(u'провалов: %d' % bad)
    io.open(os.path.join(HERE, 'nav-sync-report.txt'), 'w',
            encoding='utf-8').write(u'\n'.join(lines) + u'\n')
    sys.stdout.write('nav sync: failed=%d -> %s\n'
                     % (bad, os.path.join(HERE, 'nav-sync-report.txt')))
