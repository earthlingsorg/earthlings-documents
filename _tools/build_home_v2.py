...
BANDS = [
    ...
    # Awakened Code живьём, во всю полосу. Тёмно-синяя полоса ему и нужна:
    # терминал на чёрном - его собственное оформление, и на светлой полосе
    # экран выглядел бы дырой.
    ('navy',  'awakened', [],        u''),
]

# Сколько эссе показать рядом с экраном - по стольку из левой и правой
...
def awakened(theme, lang):
    ...
    Рамки монитора больше нет (решение Артура 2026-08-22): экран прижат к
    правому краю окна и занимает полосу по высоте и ширине настолько, насколько
    она позволяет. Причина не в украшательстве - у Awakened Code по краям две
    ленты по шесть эссе, и в маленькой рамке они не помещались: уже вся ширина
    рамки на узком мониторе уводила его в мобильную раскладку, где ленты
    прячутся под кнопку. Экран во всю полосу показывает все двенадцать.
    ...
    экрана, поэтому правило «страница читается без JS» держится: экран - это
