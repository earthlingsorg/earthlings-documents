...
def platform(theme, lang, title, lead_ps):
    ...
    return u'\n'.join([
        ...
        # Ссылка в документ и кнопка платформы - в текстовой колонке, прижаты
        # к её низу. За день это место меняли дважды, и оба решения Артура
        # верны для своего положения дел. Утром выходы унесли из колонки вниз
        # полосы: человек встречал предложение уйти раньше, чем видел, куда
        # его зовут. Вечером рамка тура стала выше колонки - слева под текстом
        # образовалась пустота, и выходы вернулись в неё. Довод «сначала
        # покажи, потом зови» при этом не нарушен: прижатые к низу, они стоят
        # на одной линии с низом рамки, то есть ниже всего, что показано.
