...
def langlist(lang):
    ...
    """
    # Девять названий стоят на белой плите внутри синей полосы (решение Артура
    # 2026-08-25). Прежде они были светлыми по тёмно-синему, и каждая карточка
    # держалась одной верхней рамкой; на белом девять письменностей читаются
    # как один разворот, а полоса остаётся синей вокруг него.
    o = [u'<div class="band-box">',
         u'<ul class="langlist" aria-label="%s">' % C.esc(C.x(lang, 'lang_switcher'))]
...
def steps(theme, lang, lead_ps):
    ...
    return u'\n'.join([
        ...
        # Шесть шагов стоят на белой плите внутри синей полосы (решение Артура
        # 2026-08-25). На тёмно-синем они читались светлым по тёмному и
        # держались одними рамками; на белом снова читаются одной таблицей.
        u'<div class="band-box">',
