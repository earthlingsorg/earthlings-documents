...
DROP_BLOCKS = [
    ...
    # справка сборки, а не текст документа: в мастере языка её нет и быть не
    # должно. Внутри только <p> и <a>, вложенного div нет - нежадный захват
    # доходит ровно до конца пометки.
    r'<div class="lag-note".*?</div>',
]
