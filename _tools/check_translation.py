...
# закрывающих делает audit_lang.py по конфигурации языка.
#
# Китайский - единственный язык, которому запись нужна по-настоящему, и она
# шире немецкой. Проверено переписью кодпойнтов по всем 23 китайским страницам
# сайта 2026-08-18: полноширинные 。，、：；（）《》 в словаре выше отсутствуют
# вовсе и проходят сами, а вот три знака в нём есть и без них весь китайский
# корпус завалил бы проверку:
#   U+2014 - китайское тире 破折号, которое набирается ДВУМЯ подряд (——);
#   U+201C и U+201D - двойные кавычки упрощённого письма, обе законные.
# Разрешение компенсируется в audit_conf/zh.py: там проверяется парность
# U+2014 (из 1047 вхождений в старых страницах 5 были одиночными - это русское
# тире, приехавшее в китайский текст как есть) и баланс кавычек. Без той
# проверки эта строка была бы дырой, а не поправкой.
