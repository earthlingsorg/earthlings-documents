...
DEFAULTS = {
    'TITLE': '',
    # Модель словоформы. Их три, и это не вкус, а устройство языков корпуса:
...
def sentences(conf, text):
    ...
    for para in re.split(r'\n\s*\n|\n(?=\s*(?:[-*]|\d+\.)\s)', body):
        ...
        if not para:
            continue
        for s in re.split(r'(?<=[.!?])\s+(?=[' + start + r'])', para):
            yield s
...
def sec_long(ctx):
    conf = ctx.conf
    print('\n=== 9. ДЛИННЫЕ ПРЕДЛОЖЕНИЯ (> %d слов) ===' % conf.LONG_SENT)
    found = 0
    for name, s in sorted(ctx.texts.items()):
