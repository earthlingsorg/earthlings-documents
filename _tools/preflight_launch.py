...
def check_continuity(tree, resolve):
    u"""Каждый адрес прежнего sitemap обязан отдаваться и сейчас.
    ...
    Почему не по умолчанию. Триста четыре запроса по сети - это секунды, а
    ...
    говорит, что живьём не спрашивал.
    """
    ...
    if not LIVE:
        ...
        return Row(name, not gone,
                   u'по дереву и nginx: адресов %d, потеряется %d '
                   u'(живой сайт - с --live)' % (len(locs), len(gone)), gone)
    ...
    return Row(name, not bad,
               u'спрошен живой сайт: адресов %d, отвечают не 200/301: %d'
               % (len(locs), len(bad)), bad)
