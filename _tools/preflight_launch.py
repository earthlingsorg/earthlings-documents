...
# --------------------------------------------------------------------------
# Живой сайт. Спрашивается только по флагу --live.
# --------------------------------------------------------------------------

# Спрашивать ли живой сайт вместо дерева файлов. Ставится из argv в main() и
# из preflight_all через run_checks(live=...).
LIVE = False
LIVE_HOST = 'earth-lings.org'
_conn = {}


def live_status(path):
    u"""Код ответа боевого сайта на адрес. HEAD, одно соединение на все.

    Зачем HEAD, а не GET: нужен код, а не тело. Триста адресов телом - это
    десятки мегабайт по сети на каждую приёмку.

    Соединение переиспользуется: триста рукопожатий TLS подряд стоят дороже
    самих запросов. Соединение рвётся (сервер закрыл keep-alive, сеть
    моргнула) - переоткрываем один раз и повторяем запрос. Второй отказ
    подряд возвращается как есть, строкой: молча превратить сетевую беду в
    «адрес потерян» нельзя, это разные новости.
    """
    for attempt in (0, 1):
        try:
            if 'c' not in _conn:
                _conn['c'] = http.client.HTTPSConnection(LIVE_HOST, timeout=20)
            c = _conn['c']
            c.request('HEAD', path, headers={
                'User-Agent': 'earthlings-preflight',
...
def load_shared_dirs():
    u"""Каталоги, которые отдаются из ПРЕЖНЕГО дерева, а не из нового корня.

    Читаются из живого конфига, а не хранятся здесь списком. Причина ровно
    та же, по которой menu_fits.py читает величины из tokens.css: копия
    разошлась бы с оригиналом при первой правке, и проверка начала бы мерить
    состояние, которого нет.

    Живой конфиг - это `earth-lings.conf`, а не `new.conf`. До подмены
    2026-09-06 читался `new.conf`, vhost черновика new.earth-lings.org, и это
    было верно: общий блок жил только там. В день подмены блок переехал в
    боевой vhost, и переехал не тем же составом - `book` и `mainpage`
    добавились. Проверка же продолжала читать черновиковый файл и об этом не
    знала: она валила два пункта из тридцати одного, обещая потерю 21 адреса
    книги, которые на живом сайте отдают 200. `new.conf` с того дня -
    рудимент, и читать из него состав боевого сайта нельзя.

    Ищем не первую попавшуюся regex-локацию, а именно ту, чьё тело уходит в
    `@shared`. В боевом конфиге есть вторая группа такого же вида - список
    языков `^/(ru|en|...)/?$`, - и она стоит ниже: `re.search` без привязки к
    `@shared` вернул бы состав общих каталогов при первой же перестановке
    блоков местами. Ошибка была бы тихой: список языков - тоже список.
    """
    p = os.path.join(NGINX, 'earth-lings.conf')
...
def check_continuity(tree, resolve):
    u"""Каждый адрес обоих sitemap обязан отдаваться сейчас.

    Sitemap два, и вопросы к ним разные. Прежний - обещание, данное раньше:
    его адреса разосланы письмами и стоят в чужих статьях, и потерять их
    нельзя. Нынешний - обещание, данное сейчас: адрес, который мы сами
    положили в карту сайта, обязан отвечать, иначе мы зовём краулера на
    ошибку. Проверка одна, потому что измерение одно.

    Второй список появился здесь 2026-09-06, и появился дорого. Пока
    ходили только по прежнему sitemap, девять библиотек документов отдавали
    404 по своему же каноническому адресу - `/documents/<язык>/`, - и ни одна
    проверка этого не видела: в прежнем sitemap их не было (до 2026-09-01 на
    них стоял noindex), а дерево файлов отвечало «index.html на месте». Файл
    и правда был на месте; отдавать его nginx отказывался.

    Спрашивает ЖИВОЙ САЙТ, а не дерево файлов, - но только по флагу --live.

    До подмены дерева хватало: новый сайт лежал в `_v2`, боевой рядом, и
    вопрос «что перестанет отдаваться, когда корень сменится» был вопросом о
    файлах. После подмены сравнение деревьев врёт, и врёт в обе стороны.
    Сегодня оно показало потерю 21 адреса книги, которые отдают 200: nginx
    берёт их из прежнего дерева через `@shared`, а проверка о нём не знала.
    Промолчать оно может так же легко: файл на месте, а адрес закрыт
    правилом location - и проверка этого не увидит.

    Живой сайт таких вопросов не задаёт: он отвечает кодом. Годным считается
    200 и 301 - редирект тоже континуитет, старый адрес ведёт на новый.

    Годным считается 200 и 301: редирект - это тоже континуитет, старый
    адрес ведёт на новый. Перенаправления всё же считаются отдельно и
    называются в отчёте: в НЫНЕШНЕМ sitemap редирект - не потеря, но и не
    норма, ради избавления от них сайтмап и отдали генератору.

    Почему не по умолчанию. Шестьсот запросов по сети - это полминуты, а
    предполёт гоняют по многу раз за сессию, в том числе там, где сети может
    не быть вовсе. Поэтому обычный прогон отвечает по дереву и честно
    говорит, что живьём не спрашивал. Цена этой честности известна: по дереву
    те девять четырёхсоток не видны и сейчас.
    """
    name = u'континуитет: адреса прежнего и нынешнего sitemap'
    files = [(u'прежний', os.path.join(SITE, 'sitemap.xml')),
             (u'нынешний', os.path.join(V2, 'sitemap.xml'))]
    locs, seen, counts = [], set(), {}
    for label, p in files:
        if not os.path.isfile(p):
            return fail(name, u'нет %s sitemap: %s' % (label, p))
        found = re.findall(r'<loc>https://earth-lings[.]org([^<]*)</loc>', read(p))
        if not found:
            return fail(name, u'в %s sitemap ни одного адреса' % label)
        counts[label] = len(found)
        for u in found:
            if u not in seen:
                seen.add(u)
                locs.append(u)
    tail = u'прежний %d, нынешний %d, разных %d' % (
        counts[u'прежний'], counts[u'нынешний'], len(locs))

    if not LIVE:
        gone = [u for u in locs if resolve(u) is None]
        return Row(name, not gone,
                   u'по дереву и nginx: %s, потеряется %d '
                   u'(живой сайт - с --live)' % (tail, len(gone)), gone)
    ...
    for u in locs:
        ...
        if st == 301:
            ...
            bad.append(u'%s -> %s' % (u, st))
    return Row(name, not bad,
               u'спрошен живой сайт: %s, отвечают не 200/301: %d%s'
               % (tail, len(bad),
                  u', перенаправлений %d' % moved if moved else u''), bad)
...
#
# Сейчас список ПУСТ, и это не оплошность. В нём стояли
# /verification/documents/{en,ru}/ - кнопки «Browse EN/RU Folder» ждали
# листинга каталога, которого нет и не будет (autoindex off). Дефект
# закрыт 2026-09-07 правкой самих страниц: четыре ссылки теперь ведут на
# якоря #english и #russian той же страницы documents/, которая и так
# перечисляет все 54 подписи скриптом. Исключение снято вместе с
# причиной: список, переживший свой дефект, замаскирует следующий на том
# же адресе.
ASSET_KNOWN_BROKEN = {}
...
def check_cross_language(tree, resolve):
    ...
    for relpath, s in sorted(tree.pages.items()):
        ...
        lang = m.group(1)
        seen += 1
        # Пометка «перевод отстаёт от мастера» (build_site_docs.lag_note_html)
        # ведёт на русский и английский текст того же документа НАМЕРЕННО:
        # это и есть её назначение. Вырезается только она, остальное тело
        # по-прежнему обязано ссылаться внутри своего языка.
        body = re.sub(r'<div class="lag-note".*?</div>', ' ', main_of(s), flags=re.S)
...
def main():
    ...
    rows = run_checks(live='--live' in sys.argv)
