...
RETIRED = {
    '17': {
        ...
        'slugs': {'ru': 'chto-dalshe',
                  ...
                  'hi': 'what-may-happen-next',
                  'ka': 'what-may-happen-next'},
    },
}

# Почему в RETIRED только 17, хотя из корпуса убирали и другое.
#
# Документы 06 «Теоретико-математическая рамка», 18 «От Декларации к Конституции
# Человечества», 21 и 25 «Доктринальный комментарий к праву на самоопределение»
# сняты 2026-07-27 коммитом 4ae8dfb4 сайта, вместе с темой
# topics/constitution-for-humanity. Это было за месяц до того, как завелось
# правило выше, поэтому в таблицу они не попали.
#
# Вносить их туда не нужно: содержание никуда не переехало, направление закрыто
# (подтверждено Артуром 2026-09-25). RETIRED существует ради содержания, которое
# сменило адрес; для содержания, которого больше нет, правильный ответ - 404, а
# редирект на неродственную страницу хуже и читателю, и поисковику.
#
# Поэтому 31 адрес документов и 7 адресов темы отдают 404 намеренно. Google
# держит их в «Не найдено (404)» - это ожидаемо и чинить нечего. Проверено
# поштучно 2026-09-25, разбор в C:\memory\_История\ОТЧЁТ_SEO_данные_Search_Console_2026-09-25.md.
#
# Осторожно на будущее: у тематических хабов механизма редиректов нет вовсе -
# карта собирается только по документам. Если хаб переименуют, старый адрес
# умрёт молча.
...
OVERRIDES = {
    ...
    # ------------------------------------------------------------------
    # Девятнадцать описаний, заведённых 2026-09-01.
    #
    # У русского WRAPPER_FROM_PAGE = True: обвязка берётся с живой страницы.
    # На боевых русских страницах description не стоит, и пустота приезжала
    # в новое дерево. Получалось так: у восьми языков описание есть, а у
    # языка-источника - у девятнадцати документов из двадцати пяти нет.
    # Поисковик в таком случае собирает сниппет сам, из первого попавшегося
    # абзаца.
    #
    # Написаны по содержанию документа, а не по названию: у каждого
    # прочитаны разделы, и в описании стоит то, что в нём действительно
    # есть. Название в выдаче и так видно из title, повторять его вторично
    # незачем.
    # ------------------------------------------------------------------
    '03': {
        'description': 'Этика Earthlings: как народ разбирает разногласия - прямой '
                       'диалог, медиация, восстановительный подход вместо '
                       'карательного. Оснований для каких-либо мер этот документ '
                       'не устанавливает.',
    },
    '05': {
        'description': 'Устав Earthlings - организационный документ народа: DAO '
                       'Ассамблея как единственный орган управления, типы '
                       'голосований, техническая инфраструктура, этический надзор '
                       'и закрытый перечень оснований для мер.',
    },
    '07': {
        'description': 'DAO Earthlings - институциональное ядро управления народом: '
                       'как принимаются коллективные решения, распределяются '
                       'ресурсы и согласуются действия участников и сот без центра.',
    },
    '08': {
        'description': 'Соты Earthlings - базовая организационная единица народа: '
                       'малые команды, через которые идут проекты, продукты и '
                       'сервисы, и горизонтальные связи между участниками.',
    },
    '09': {
        'description': 'Казначейство Earthlings - финансовый механизм народа: чем '
                       'распоряжается голосование DAO Ассамблеи, на что идут '
                       'средства и какими процедурами это можно проверить со '
                       'стороны.',
    },
    '10': {
        'description': 'Earthlings Coin - внутренняя расчётная единица народа: как '
                       'вклад участника делается измеримым, а общие средства - '
                       'распределяемыми без посредников. Голосом коин не является '
                       'и веса в решениях не даёт.',
    },
    '11': {
        'description': 'Независимый Совет Earthlings - консультативный орган '
                       'этического надзора: наблюдает, насколько народ следует '
                       'собственным ценностям. Решений за народ не принимает и '
                       'власти над ним не имеет.',
    },
    '12': {
        'description': 'Цифровая платформа Earthlings: подтверждение личности и '
                       'непередаваемый паспорт, личный кабинет, соты и проектный '
                       'поток, голосование и делегирование. Один earthling - один '
                       'голос.',
    },
    '14': {
        'description': 'Путь earthling - практическое руководство по вступлению: от '
                       'чтения Декларации до паспорта. Национальность, раса, '
                       'религия, пол, гражданство и место проживания на вступление '
                       'не влияют.',
    },
    '15': {
        'description': 'SBT-паспорт earthling - непередаваемое цифровое '
                       'удостоверение принадлежности к народу: что оно '
                       'подтверждает, как выдаётся после проверки личности и '
                       'почему его нельзя продать или передать.',
    },
    '16': {
        'description': 'Политика биометрической верификации Earthlings: как '
                       'подтверждается, что за голосом стоит один живой человек, '
                       'что происходит с биометрическими данными и почему система '
                       'непригодна для слежения.',
    },
    '19': {
        'description': 'Дорожная карта переходного периода: этапы, которыми '
                       'Earthlings идут от основания к полной децентрализации, и то, '
                       'как за этим можно наблюдать со стороны. Децентрализация - '
                       'цель, а не отправная точка.',
    },
    '20': {
        'description': 'Учредительный период: как народ, создающий себя сам, '
                       'принимает свой учредительный текст - что открыто для '
                       'предложений, что не обсуждается и что будет, если вы не '
                       'согласны с принятым текстом.',
    },
    '22': {
        'description': 'Юридическая информация: правовая модель народа Earthlings и '
                       'её пределы, правовые интерфейсы для внешнего '
                       'взаимодействия, права на тексты корпуса и прямой перечень '
                       'того, чем народ не является.',
    },
    '23': {
        'description': 'О нас: кто написал корпус Earthlings, как устроено '
                       'управление, откуда берутся деньги, как раскрывается казна и '
                       'что любой человек может проверить публично.',
    },
    '28': {
        'description': 'Политика конфиденциальности народа Earthlings: какие данные '
                       'собираются, кто за них отвечает, что попадает в '
                       'распределённый реестр и как воспользоваться правами '
                       'субъекта данных.',
    },
    '29': {
        'description': 'Пользовательское соглашение народа Earthlings: условия '
                       'принадлежности и регистрации, ответственность за учётную '
                       'запись и природа самого соглашения. Написано не корпорацией '
                       'для защиты корпорации.',
    },
    '30': {
        'description': 'Как возникает субъект права: тезисы о добровольном '
                       'конституировании народа. Вопрос не об Earthlings, а о '
                       'международном праве - чего оно ещё не разработало и почему '
                       'этот пробел не делает народ неправомерным.',
    },
    '31': {
        'description': 'Рабочая повестка: разбор нынешнего мироустройства как старой '
                       'операционной системы - перечень сбоев, микроядро вместо '
                       'монолита, право выхода и субсидиарность. Модель как пример, '
                       'а не чертёж для внедрения.',
    },
}
...
OVERRIDES_KA = {
    ...
    # Записи 02 не было вовсе, и грузинская страница уезжала без описания -
    # единственная такая из девяти. Документ 02 заведён 2026-08-21, восемь
    # языков получили содержательное описание, грузинский пропустили.
    # Здесь стоит шаблон, как у тринадцати других грузинских документов:
    # сочинять содержательное описание на языке, которого не знает ни автор
    # правки, ни тот, кто её принимает, значит завести непроверенную прозу.
    # Перевод описания - задача следующего грузинского прохода.
    '02': {'description': _OFFICIAL_KA % 'სამოქალაქო ხმა'},
    # Описания приложений - по подзаголовку самого приложения (блок 1), а
    # og - по образцу статусной врезки: «დოკუმენტის «სამოქალაქო ხმა» დანართი».
    '02a': {'description':
            'სამოქალაქო ხმის ხუთი ხარვეზი, საქმე თვრამეტი მილიონი გაყალბებული კომენტარის '
            'შესახებ და მათემატიკა, რომლის გამოც გაფანტული უმრავლესობა ორგანიზებულ '
            'უმცირესობასთან აგებს.',
            'og_description':
            'დოკუმენტის «სამოქალაქო ხმა» დანართი: რატომ არის დათვლა გაფუჭებული.'},
    '02b': {'description':
            'ექვსი შემთხვევა, როცა ადამიანებს არავითარი უფლებამოსილება არ ჰქონდათ - და '
            'მათ მაინც ანგარიშს უწევდნენ; და ორი ცდა, რომლებიც ყველაზე ახლოს მივიდნენ '
            'სიღრმისა და მასშტაბის შეერთებასთან.',
            'og_description':
            'დოკუმენტის «სამოქალაქო ხმა» დანართი: გავლენა უფლებამოსილების გარეშე.'},
    '02c': {'description':
            'ხუთი მიზეზი, რის გამოც საერთო ნების შემოწმებად გაზომვას ხალხი სჭირდება, მის '
            'გარეშე აგებული უმსხვილესი პროექტის შემთხვევა და ცხრა გზა, რომლითაც ეს '
            'ყველაფერი შეიძლება არ შედგეს.',
            'og_description':
            'დოკუმენტის «სამოქალაქო ხმა» დანართი: რატომ სჭირდება ხელსაწყოს ხალხი.'},
...
OVERRIDES_AR = {
    ...
    # Описания приложений - по подзаголовку самого приложения (блок 1), а og - по
    # образцу статусной врезки: «ملحق بوثيقة «الصوت المدني»». Огласовок нет, кроме
    # шадды в `العدّ` - она стоит так и в названии приложения, и в og документа 02.
    '02a': {'description':
            'خمسة أعطال في الصوت المدني، وقضية ثمانية عشر مليون تعليق مزيف، '
            'والرياضيات التي تجعل الأكثرية المشتتة تخسر أمام الأقلية المنظمة.',
            'og_description':
            'ملحق بوثيقة «الصوت المدني»: لماذا العدّ معطوب.'},
    '02b': {'description':
            'ست حالات لم تكن فيها للناس أي صلاحيات - ومع ذلك حُسب لهم حساب؛ '
            'وتجربتان اقتربتا أكثر من غيرهما من الجمع بين العمق والاتساع.',
            'og_description':
            'ملحق بوثيقة «الصوت المدني»: التأثير دون صلاحيات.'},
    '02c': {'description':
            'خمسة أسباب يحتاج من أجلها قياس الإرادة المشتركة القابل للتحقق إلى شعب، '
            'وحالة أكبر مشروع بُني بدونه، وتسع طرق قد لا يتم بها هذا كله.',
            'og_description':
            'ملحق بوثيقة «الصوت المدني»: لماذا يحتاج المقياس إلى شعب.'},
...
_OFFICIAL_HI = '%s - Earthlings जन का आधिकारिक दस्तावेज़।'

# Хинди. Описания - текст, а не настройка: переведены, а не скопированы.
#
# Три вещи, которых нет у других языков.
#
# Первое: «народ» здесь जन, а не जनता и не लोग. जनता в Конституции Индии
# стоит только как «साधारण जनता» (широкая публика) и вдобавок носит шлейф
# названий индийских партий; लोग значит «люди» и счётности не принимает.
# Разбор - hi/_СЛОВАРЬ_РЕШЕНИЙ.md, раздел 2.1.
#
# Второе: «официальный» здесь शासकीय (Конституция, ст. 348(2), 347), а не
# सरकारी. Хинди-текст ВДПЧ называет словом सरकारी пять АУТЕНТИЧНЫХ языков,
# и в описаниях документов оно стёрло бы то самое различение, на котором
# держится статья 13 Декларации.
#
# Третье, и это касается всех языков: **в описании документа 20 дат нет**.
# Даты учредительного периода живут в тексте документа 20, где у них один
# источник. В описаниях они за два месяца разъезжались с текстом дважды
# (коммит 87721ca сдвинул период на 45 дней, описания остались прежними -
# пункт 43 списка правок мастера). Решение Артура 2026-09-24: описание
# говорит, что обсуждается и как участвовать, а дат не называет. Замок -
# _check_no_dates_in_20 ниже, он роняет сборку.
OVERRIDES_HI = {
    '01': {'description': _OFFICIAL_HI % 'Earthlings घोषणा'},
    '02': {'description':
           'नागरिक आवाज़ को आज बिना उसके सार पर विवाद किए क्यों अनसुना किया जा '
           'सकता है, Earthlings इसके बदले क्या बना रहे हैं, और ऐसी आवाज़ किन '
           'सीढ़ियों से वज़न पाती है।',
           'og_description':
           'टूटी हुई चीज़ गिनती है, माध्यम नहीं। इसका क्या अर्थ है और Earthlings '
           'इसके साथ क्या करते हैं।'},
    '02a': {'description':
            'नागरिक स्वर की पाँच ख़राबियाँ, अठारह मिलियन जाली टिप्पणियों का मामला, '
            'और वह गणित जिसके कारण बिखरा हुआ बहुमत संगठित अल्पमत से हार जाता है।',
            'og_description':
            '«नागरिक स्वर» का परिशिष्ट: गिनती क्यों टूटी हुई है।'},
    '02b': {'description':
            'छह मामले, जिनमें लोगों के पास कोई शक्ति नहीं थी - और फिर भी उनका '
            'लिहाज़ किया गया; और वे दो प्रयोग, जो गहराई को पैमाने से जोड़ने के '
            'सबसे नज़दीक पहुँचे।',
            'og_description':
            '«नागरिक स्वर» का परिशिष्ट: बिना शक्तियों के प्रभाव।'},
    '02c': {'description':
            'पाँच कारण, जिनसे सामूहिक इच्छा का जाँचा जा सकने वाला मापन जन पर आ '
            'टिकता है; उस सबसे बड़ी परियोजना का मामला, जो उसके बिना बनी; और नौ '
            'तरीक़े, जिनसे यह सब नहीं बन पाएगा।',
            'og_description':
            '«नागरिक स्वर» का परिशिष्ट: यंत्र जन पर आकर क्यों टिकता है।'},
    '03': {'description': _OFFICIAL_HI % 'Earthlings की नैतिकता'},
    '04': {'description':
           'Earthlings जन का विधिक आधार: संगम या संघ बनाने की स्वतंत्रता, '
           'आत्मनिर्णय का अधिकार, जन के लक्षण और अंतरराष्ट्रीय विधि के खुले '
           'प्रश्न - स्रोतों सहित, और इस स्पष्ट कथन के साथ कि विधि ने अब तक '
           'किसका निपटारा नहीं किया।',
           'og_description':
           'संगम बनाने की स्वतंत्रता, आत्मनिर्णय, जन के लक्षण, और वह जो '
           'अंतरराष्ट्रीय विधि ने अब तक तय नहीं किया।'},
    '05': {'description': _OFFICIAL_HI % 'Earthlings चार्टर'},
    '07': {'description': _OFFICIAL_HI % 'Earthlings DAO: सिद्धांत, संरचना और शासन'},
    '08': {'description': _OFFICIAL_HI % 'Earthlings कोशिकाएँ: परियोजनाओं और सहयोग की प्रणाली'},
    '09': {'description': _OFFICIAL_HI % 'Earthlings का साझा कोष'},
    '10': {'description': _OFFICIAL_HI % 'Earthlings Coin: आंतरिक गणना इकाई'},
    '11': {'description': _OFFICIAL_HI % 'Earthlings की स्वतंत्र परिषद'},
    '12': {'description': _OFFICIAL_HI % 'Earthlings का डिजिटल मंच'},
    '14': {'description': _OFFICIAL_HI % 'earthling का मार्ग'},
    '15': {'description': _OFFICIAL_HI % 'earthling का SBT पासपोर्ट'},
    ...
    '26': {'description':
           'Earthlings जन की संरचना पर आपत्तियाँ - पृथक्करण, प्रभुता, DAO में '
           'धनिकतंत्र, प्रवेश फीस, अपरिवर्तनीय केंद्रक और किसी के नाम पर बोलने '
           'के अधिकार को लेकर - उत्तरों सहित, और उस सूची के साथ जिसे हम खंडित '
           'नहीं मानते।',
           'og_description':
           'Earthlings जन की संरचना पर आपत्तियाँ और उनके उत्तर, उनके साथ जिन्हें '
           'हम खंडित नहीं मानते।'},
    '27': {'description':
           'Earthlings जन शासन, अर्थव्यवस्था, पहचान और नैतिकता के बारे में '
           'सामान्य प्रश्नों के उत्तर देता है: यह कैसे काम करता है, वास्तव में '
           'नियंत्रण किसका है, डाटा की रक्षा कैसे होती है।',
           'og_description':
           'Earthlings जन शासन, अर्थव्यवस्था, पहचान और नैतिकता के सामान्य '
           'प्रश्नों के उत्तर देता है।'},
    '28': {'description': _OFFICIAL_HI % 'Earthlings जन की गोपनीयता नीति'},
    '29': {'description': _OFFICIAL_HI % 'Earthlings जन की उपयोग शर्तें'},
    '30': {'description':
           'Earthlings जन के प्रतिपादन: अंतरराष्ट्रीय विधि ने सामूहिक व्यक्तियों '
           'के अस्तित्व का सिद्धांत क्यों विकसित किया, पर उनके स्वैच्छिक उद्भव '
           'का लगभग नहीं - और यह रिक्ति जन के उद्भव को अवैध क्यों नहीं बनाती।'},
    '31': {'description':
           'Earthlings जन की कार्य-सूची: आने वाली विश्व-व्यवस्था के एक संभव '
           'प्रतिरूप का विशेषज्ञ विश्लेषण, प्रचालन तंत्र के रूपक के सहारे। '
           'राज्यों का पूरक, उनका विलोप नहीं।'},
    '32': {'description':
           'हम अभी कहाँ हैं: Earthlings कौन-सा कोड और कौन-सा डाटा प्रकाशित करते '
           'हैं, क्या बंद है, किस कारण से, और कोई भी व्यक्ति स्वयं किसकी जाँच '
           'कर सकता है।',
           'og_description':
           'Earthlings कौन-सा कोड और कौन-सा डाटा प्रकाशित करते हैं, क्या बंद है '
           'और किस कारण से।'},
}

OVERRIDES_BY_LANG = {'ru': OVERRIDES, 'en': OVERRIDES_EN, 'de': OVERRIDES_DE,
                     'fr': OVERRIDES_FR, 'es': OVERRIDES_ES, 'ka': OVERRIDES_KA,
                     'zh': OVERRIDES_ZH, 'ar': OVERRIDES_AR, 'hi': OVERRIDES_HI}


def _check_no_dates_in_20():
    """Описание документа 20 не называет дат ни на одном языке (см. комментарий
    у OVERRIDES_HI). Год ловится и арабскими цифрами, и китайскими."""
    assert len(OVERRIDES_BY_LANG) == 9
    for lang, ov in OVERRIDES_BY_LANG.items():
        for key in ('description', 'og_description'):
            s = ov.get('20', {}).get(key, '')
            m = re.search(u'20[2-3][0-9]|二〇[二三]', s)
            assert not m, (u'описание документа 20 (%s, %s) называет дату %r - '
                           u'даты живут в тексте документа' % (lang, key, m.group(0)))


_check_no_dates_in_20()
...
# Хинди-страницы - самый ясный случай из всех девяти. Их 22 против 25
# мастеров, и перепись показала, что термины в них разошлись внутри самого
...
# Грузинский вернут в документ 02 2026-08-26: мастер ka/02-civic-voice.md
# закоммичен, заморозка снята. Тем же списком фильтруется меню.
# ХИНДИ ДОБАВЛЯЕТСЯ СЮДА ТОЛЬКО ВМЕСТЕ СО СЛАГАМИ, а не раньше. Правило
# «слаги вносить, когда все 25 мастеров на месте» распространяется и на эту
# таблицу, и это выяснилось дорого: 2026-08-26 хинди был вписан сюда до
# слагов, и пересборка немедленно поставила в переключатель языков ссылку
# `/documents/hi/hi32.html` - на файл, которого не существует. Документы 02,
# 20 и 32 в прежнем круге на хинди не переводились вовсе, поэтому запасного
# числового адреса у них нет: до слагов сборщик законно уводил читателя на
# `/documents/hi/index.html`, то есть в библиотеку, а после правки - в 404.
#
...
LANGS_BY_DOC = {'02': ['ar', 'de', 'en', 'es', 'fr', 'hi', 'ka', 'ru', 'zh'],
                # Приложения заведены 2026-09-23. К немецкому, испанскому,
                # французскому, китайскому, хинди, грузинскому и арабскому они пришли вместе с
                # сокращённой редакцией 02: приложение и резка 02 - одно и то
                # же изменение, порознь их выкладывать нельзя. С арабским
                # (2026-09-24) приложения собирают все девять языков.
                '02a': ['ar', 'de', 'en', 'es', 'fr', 'hi', 'ka', 'ru', 'zh'],
                '02b': ['ar', 'de', 'en', 'es', 'fr', 'hi', 'ka', 'ru', 'zh'],
                '02c': ['ar', 'de', 'en', 'es', 'fr', 'hi', 'ka', 'ru', 'zh'],
...
UI = {
    ...
    # Хинди читается слева направо, стрелки обычные - зеркалить их, как в
    # арабском, не нужно. Заглавных букв в деванагари нет, поэтому вопроса
    # о регистре строк здесь не возникает вовсе.
    'hi': {
        'toc': 'विषय-सूची',
        'prev': '← पीछे',
        'next': 'आगे →',
        'nav_aria': 'दस्तावेज़ों में नेविगेशन',
        # Совпадает знак в знак с EXTRA['all_docs']['hi'] в chrome.py:
        # одна и та же надпись в двух местах обязана быть одной строкой.
...
# ---------------------------------------------------------------- отставание перевода
# Правки мастера вносятся иногда только в русский и английский (решение Артура
# 2026-09-14: заход правок мастера 15-17). Перевод на другом языке тогда
# расходится с мастером, и читатель должен это видеть на самой странице, а не
# узнавать из реестра. Данные - `lagging_translations.json` рядом: документ,
# языки, дата внесения правок. Язык догнал мастер - убрать его из записи;
# вручную в HTML пометку не вписывать, сборка её перезапишет.
#
# Дата - ISO (2026-09-17): месяцы семи языков не переводятся и не путаются,
# а в арабском дата изолирована <bdi>, иначе цифры и дефисы переставит
# двунаправленный алгоритм. Подписи ссылок - на языке цели ссылки («Русский
# текст», «English text») с атрибутом lang: читатель ищет свой язык по его
# собственному имени, как в переключателе языков.
LAGGING_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'lagging_translations.json')
LAG_LINK_LABEL = {'ru': 'Русский текст', 'en': 'English text'}


def load_lagging():
    with io.open(LAGGING_PATH, encoding='utf-8') as f:
        data = json.load(f)
    text = data['text']
    for rec in data['lagging']:
        assert re.fullmatch(r'\d{4}-\d{2}-\d{2}', rec['date']), rec
        for lang in rec['langs']:
            assert lang in ALL_LANGS and lang not in ('ru', 'en'), (
                'пометка об отставании - только для переводов, не для %r' % lang)
            assert text.get(lang, '').count('{date}') == 1, (
                'нет текста пометки об отставании для языка %r' % lang)
    return data


LAGGING = load_lagging()


def lag_note_html(num, lang):
    """Пометка «перевод отстаёт от мастера» под шапкой документа, или ''."""
    recs = [r for r in LAGGING['lagging'] if r['doc'] == num and lang in r['langs']]
    if not recs:
        return ''
    date = min(r['date'] for r in recs)          # самые ранние не догнанные правки
    text = html.escape(LAGGING['text'][lang], quote=False).replace(
        '{date}', '<bdi>%s</bdi>' % date)
    links = ' '.join('<a href="%s" lang="%s" hreflang="%s">%s</a>' % (
        doc_href(num, code), code, code, LAG_LINK_LABEL[code]) for code in ('ru', 'en'))
    return ('<div class="lag-note" role="note"><p>%s</p>'
            '<p class="lag-note-links">%s</p></div>' % (text, links))
...
def build_doc_v2(num, doc, head, toc, body, titles, lang):
    ...
    return '\n'.join([
        ...
        '<div class="sheet">',
        # Пометка приклеена к шапке, а не отдельным элементом списка: у
        # документа без отставания пустая строка иначе меняла бы каждую из
        # 225 страниц одной лишней строкой.
        head + lag_note_html(num, lang), toc, body,
        '</div>',
