...
HARD_LOCKS = [
    ...
    # Замок принадлежности, поставлен 2026-08-21. Шаблон намеренно узкий.
    # `adhésion` под него НЕ попадает и попадать не должно: французский
    # словарь держит его за ДЕЙСТВИЕМ вступления, которое русский сохраняет
    # («adhésion au peuple» - 4 законных вхождения). Запрещено называть
    # человека членом народа и называть взнос членским.
    (r'\bmembres? du peuple\b|\bcotisation de membre\b',
     'принадлежность к народу - appartenance; membre du peuple запрещено'),
]
