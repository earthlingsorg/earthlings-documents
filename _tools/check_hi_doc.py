...
# запускают проверку одновременно. С общим именем они затирали бы отчёты друг
# друга, и каждый читал бы чужой - то есть проверка показывала бы «чисто» на
# файле, которого не проверяла.
OUT = os.path.join(REPO, '_tools', '_doc-report-hi-%s.txt' % NUM)
