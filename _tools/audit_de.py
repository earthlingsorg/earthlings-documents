...
SOFT_LOCKS = [
    ...
    # законно стоит в отрицании («не утверждает, что даёт право говорить от
    # имени человечества», документ 04) и внутри приводимого возражения
    # (документ 26). Жёсткий замок на ней даёт постоянный шум и перестаёт быть
    # замком; мягкий печатает список, и утвердительное употребление в нём
    # видно сразу. Проверять каждое вхождение: отрицание и цитата - законны,
    # утверждение - дефект.
